...
const isEmployee = computed(() => userType.value === "Empleado");
const isSpecialist = computed(() => userType.value === "Especialista");

// Permisos para reportes
const canAccessReports = computed(() => isAdmin.value || isEmployee.value || isSpecialist.value);
...
onMounted(() => {
  ...
  window.addEventListener("storage", (event) => {
    if (
      event.key &&
      ["userId", "username", "userEmail", "userType", "fullName"].includes(event.key)
    ) {
      loadUserData();
    }
  });
...
<template>
  <v-app-bar
    height="80"
    color="teal-darken-4"
    image="https://eu-images.contentstack.com/v3/assets/blt6d90778a997de1cd/blt70f7629113e2f0fb/675b51d01f4cab9b998e3192/Olena_Ivanova-open-source-tools-shutterstock.jpg?width=1280&auto=webp&quality=95&format=jpg&disable=upscale"
  >
  ...
  <v-navigation-drawer v-model="drawer" temporary>
    ...
    <v-list>
      ...
      <template v-if="isEmployee">
        <v-list-subheader class="text-blue-accent-2 font-weight-bold">
      ...
      <template v-if="isSpecialist">
        <v-list-subheader class="text-purple-accent-2 font-weight-bold">
